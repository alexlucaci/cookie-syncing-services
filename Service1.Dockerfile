# Build the Go Binary.
FROM golang:1.14.4 as build-service-1
ENV CGO_ENABLED 0
ARG PACKAGE_NAME
ARG PACKAGE_PREFIX
...
WORKDIR /service/app/partner-service-1/
RUN go build
...
ARG PACKAGE_NAME
ARG PACKAGE_PREFIX
COPY --from=build-service-1 /service/app/admin/admin /app/admin
COPY --from=build-service-1 /service/app/partner-service-1/partner-service-1 /app/main
COPY --from=build-service-1 /service/start-partner-service-1.sh /app/start.sh
