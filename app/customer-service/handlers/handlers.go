...
func Service(shutdown chan os.Signal, log *log.Logger, service1Url string) http.Handler {
	app := web.NewApp(shutdown, mid.Logger(log), mid.Errors(log), mid.Panics(log))

	fh := homeHandlers{service1Url: service1Url}
	app.Handle(http.MethodGet, "/", fh.home)
